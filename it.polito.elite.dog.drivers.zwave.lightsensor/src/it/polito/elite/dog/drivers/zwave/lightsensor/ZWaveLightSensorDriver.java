...
public class ZWaveLightSensorDriver implements Driver, ManagedService
{
	...
	public static String LOG_ID = "[ZWaveLightSensorDriver]: ";
	
	// String identifier for driver id
	public static final String DRIVER_ID = "ZWave_ZWaveLightSensorDriver_driver";
	...
	private Vector<ZWaveLightSensorDriverInstance> connectedDrivers;
	...
	private Set<String> lightDeviceCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveLightSensorDriverInstance>();
		
		// initialize the set of implemented device categories
		lightDeviceCategories = new HashSet<String>();
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveLightSensorDriver.class.getClassLoader().loadClass(
						deviceCategory)))
				{
					...
					if (deviceCategory != null)
					{
						if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
								&& (lightDeviceCategories.contains(deviceCategory))
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveLightSensorDriverInstance driverInstance = new ZWaveLightSensorDriverInstance(network.get(), device,
	...
	{
		for (Class<?> devCat : ZWaveLightSensorDriverInstance.class.getInterfaces())
		{
			lightDeviceCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						+ " not defined in configuraton file for " + ZWaveLightSensorDriver.class.getName());
			}
