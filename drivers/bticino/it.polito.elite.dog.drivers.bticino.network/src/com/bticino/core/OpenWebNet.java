...
/**
 * Description: Class to manage the OPEN protocol
 * 
 */
public class OpenWebNet implements InterfaceOpenWebNet
{
	...
	/**
	 * Costruttore Crea una NULL_FRAME
	 * 
	 */
	public OpenWebNet()
	{
	 ...
	 * *#chi*dove#livello#interfaccia*#grandezza*val_1*val_2*....*val_n##
	 * 
	 * @param who
	 *            Campo chi
	 * @param where
	...
	private void isCorrect()
	{
		...
		if ((frameOpen.charAt(0) != '*') || (lengthFrameOpen > MAX_LENGTH_OPEN)
				|| (frameOpen.charAt(lengthFrameOpen - 1) != '#') || (frameOpen.charAt(lengthFrameOpen - 2) != '#'))
		{
		...
		if (frameOpen.charAt(1) != '#')
		{
			...
			// estraggo i vari campi della frame open nella prima modalità
		...
		// frame richiesta state ...
		// *#chi*dove##
		if (sup.charAt(0) != '*')
		{
			// System.out.println("frame state");
			tipoFrame = STATE_FRAME;
			// estraggo i vari campi della frame open nella prima modalit�
			// (chi+indirizzo e dove+livello+interfaccia)
			assegnaChiDove();
			// estraggo gli eventuali valori di indirizzo
			assegnaIndirizzo();
	...
	/**
	 * Assegna chi dove e la grandezza richiesta per le frame di richiesta
	 * grandezze
	 * 
	 */
	private void assegnaChiDoveGrandezza()
	{
		...
		else
		{
			...
			else
			{
				...
				// VALORI
				sup = null;
				sup = frameOpen.substring(2 + chi.length() + 1 + dove.length() + 1 + grandezza.length() + 1,
						frameOpen.length());
				// sprintf(sup, "%s",
				// frameOpen+2+strlen(chi)+1+strlen(dove)+1+strlen(grandezza)+1);
				j = 0;
	...
	/**
	 * Assegna chi e dove per le frame di richiesta state
	 * 
	 */
	...
	private void assegnaChiCosaDoveQuando()
	{
		...
		// CHI
		// copio in temp la frame open togliendo il primo carattere, dovrebbe
		// essere *
		try
		{
			...
			else
			{
				...
				else
				{
					...
					// QUANDO
					sup = null;
					sup = frameOpen.substring(1 + chi.length() + 1 + cosa.length() + 1 + dove.length() + 1,
							frameOpen.length());
					if (contains(sup, '*') == false)
					{
	...
	private void assegnaChiDoveGrandezzaValori()
	{
		...
		else
		{
			...
			else
			{
				...
				// VALORI
				sup = null;
				sup = frameOpen.substring(2 + chi.length() + 1 + dove.length() + 2 + grandezza.length() + 1,
						frameOpen.length());
				// sprintf(sup, "%s",
				// frameOpen+2+strlen(chi)+1+strlen(dove)+2+strlen(grandezza)+1);
				j = 0;
		...
		if (!dove.equals(""))
		{
			...
			if (contains(sup, '#') != false)
			{
				...
				if (contains(sup, '#') != false)
				{
					...
					// sprintf(interfaccia, "%s",
					// orig+strlen(dove)+1+strlen(livello)+1);
	...
	private void assegnaIndirizzo()
	{
		...
		if (contains(sup, '#') != false)
		{
			...
			else
			{
				...
				// sprintf(sup, "%s",
				// orig+strlen(chi)+1+strlen(indirizzo[0])+1);
				if (contains(sup, '#') != false)
				{
					// indirizzo IP
					i = 0;
					...
					sup = null;
					sup = orig.substring(chi.length() + 1 + indirizzo[0].length() + 1 + indirizzo[1].length() + 1,
							orig.length());
					// sprintf(sup, "%s",
					// orig+strlen(chi)+1+strlen(indirizzo[0])+1+strlen(indirizzo[1])+1);
					if (contains(sup, '#') != false)
					{
						// indirizzo IP
						i = 0;
						...
						sup = null;
						sup = orig.substring(chi.length() + 1 + indirizzo[0].length() + 1 + indirizzo[1].length() + 1
								+ indirizzo[2].length() + 1, orig.length());
						// sprintf(sup, "%s",
						// orig+strlen(chi)+1+strlen(indirizzo[0])+1+strlen(indirizzo[1])+1+strlen(indirizzo[2])+1);
						if (contains(sup, '#') == false)
						{
							// indirizzo IP
							if (sup.length() != 0)
