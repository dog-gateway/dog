...
public class ZWaveDoorWindowSensorDriver implements Driver, ManagedService
{
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if (deviceCategory != null)
		{
			...
			{
				matchValue = WindowSensor.MATCH_MANUFACTURER
						+ WindowSensor.MATCH_TYPE;
			}
			...
			throws Exception
	{
		...
		ZWaveDoorWindowSensorDriverInstance driverInstance = new ZWaveDoorWindowSensorDriverInstance(
				network.get(),
				device,
	...
	{
		for (Class<?> devCat : ZWaveDoorWindowSensorDriverInstance.class
				.getInterfaces())
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME,
						ZWaveInfo.PROPERTY_UPDATETIME
								...
								+ ZWaveDoorWindowSensorDriver.class.getName());
			}
