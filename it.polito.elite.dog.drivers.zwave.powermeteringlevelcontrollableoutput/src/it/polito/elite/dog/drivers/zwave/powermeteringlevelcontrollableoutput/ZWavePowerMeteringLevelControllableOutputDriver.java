...
public class ZWavePowerMeteringLevelControllableOutputDriver implements Driver, ManagedService
{
	...
	public static String LOG_ID = "[ZWavePowerMeteringLevelControllableOutputDriver]: ";
	
	// String identifier for driver id
	public static final String DRIVER_ID = "ZWave_ZWavePowerMeteringLevelControllableOutput_driver";
	...
	private Vector<ZWavePowerMeteringLevelControllableOutputDriverInstance> connectedDrivers;
	...
	private Set<String> powerMeteringLevelControllableDeviceCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWavePowerMeteringLevelControllableOutputDriverInstance>();
		
		// initialize the set of implemented device categories
		powerMeteringLevelControllableDeviceCategories = new HashSet<String>();
	...
	public int match(ServiceReference reference) throws Exception
	{
		...
		if (deviceCategory != null)
		{
			if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
					&& (powerMeteringLevelControllableDeviceCategories.contains(deviceCategory))
					&& (this.gateway.get().isGatewayAvailable(gateway)))
			{
				matchValue = PowerMeteringLevelControllableOutput.MATCH_MANUFACTURER + PowerMeteringLevelControllableOutput.MATCH_TYPE;
			}
	...
	public String attach(ServiceReference reference) throws Exception
	{
		...
		ZWavePowerMeteringLevelControllableOutputDriverInstance driverInstance = new ZWavePowerMeteringLevelControllableOutputDriverInstance(network.get(),
				(ControllableDevice) context.getService(reference), Integer.parseInt(sNodeID), instancesId, this.gateway
						.get().getSpecificGateway(gateway).getNodeInfo().getDeviceNodeId(), updateTimeMillis,
				context);
	...
	{
		for (Class<?> devCat : ZWavePowerMeteringLevelControllableOutputDriverInstance.class.getInterfaces())
		{
			powerMeteringLevelControllableDeviceCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						+ " not defined in configuraton file for " + ZWavePowerMeteringLevelControllableOutputDriver.class.getName());
			}
