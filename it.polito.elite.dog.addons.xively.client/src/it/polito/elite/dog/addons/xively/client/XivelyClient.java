...
public class XivelyClient implements ManagedService, EventHandler
{
	...
	protected void activate(BundleContext ctx)
	{
		...
		// log the activation
		this.logger.log(LogService.LOG_INFO, XivelyClient.logId + "Activated XIVELYOutlet");
	...
	protected void deactivate()
	{
		...
		this.logger
				.log(LogService.LOG_INFO, XivelyClient.logId + "Deactivated XIVELYOutlet, queues are shutting down.");
	}
	...
	{
		// filter spChains notifications only
		...
		{
			// debug
			this.logger.log(LogService.LOG_DEBUG, XivelyClient.logId + "Received new measure " + event.getTopic());
			...
			if (eventContent instanceof EventNotification)
			{
				...
				GenericEvent innerEvent = (GenericEvent) ((EventNotification) eventContent).getEvent();
				...
				if (streamName.matches("(feed[.][^.^-]+[-]stream[.][^.^-]++)|(stream[.][^.^-]++[-]feed[.][^.^-]+)"))
					eventFeed = this.getFeedFrom(innerEvent, this.defaultEventFeed);
				
				// handle event notifications
				this.handleRestNotification(eventFeed, innerEvent, this.waitingEventList, this.eventQueue);
			...
			else if (eventContent instanceof AlertNotification)
			{
				...
				{
					// extract the inner event
					GenericEvent innerEvent = (GenericEvent) ((AlertNotification) eventContent).getAlert();
					
					// check if the feed name refers to a cosm feed
					String streamName = innerEvent.getStreamName();
					if (streamName.matches("(feed[.][^.^-]+[-]stream[.][^.^-]++)|(stream[.][^.^-]++[-]feed[.][^.^-]+)"))
						alertFeed = this.getFeedFrom(innerEvent, this.defaultAlertFeed);
					
					// handle event notifications
					this.handleRestNotification(alertFeed, innerEvent, this.waitingEventList, this.eventQueue);
				}
				else
				{
					// handle alert notifications not based on events
					this.handleRestNotification(alertFeed,
							(GenericEvent) ((AlertNotification) eventContent).getAlert(), this.waitingAlertList,
							this.alertQueue);
				}
			}
	...
	private Integer getFeedFrom(GenericEvent innerEvent, Integer baseFeedURL)
	{
		...
		if (feedData.length > 1)
		{
			...
			{
				// add the feed id
				feedId = Integer.valueOf(feedData[0].substring(feedData[0].indexOf(".") + 1, feedData[0].length()));
				// remove the feed id from the event stream name
				innerEvent.setStreamName(feedData[1].substring(feedData[1].indexOf(".") + 1, feedData[1].length()));
			}
			else
			{
				// add the feed id
				feedId = Integer.valueOf(feedData[1].substring(feedData[1].indexOf(".") + 1, feedData[1].length()));
				// remove the feed id from the event stream name
				innerEvent.setStreamName(feedData[0].substring(feedData[0].indexOf(".") + 1, feedData[0].length()));
			}
		}
	...
	public void updated(Dictionary<String, ?> properties) throws ConfigurationException
	{
		if (properties != null)
		{
			...
			this.productSecret = (String) properties.get(XivelyClient.XIVELY_PRODUCT_SECRET);
			
			// get the api key, if needed
			this.key = (String) properties.get(XivelyClient.XIVELY_KEY);
			
			// get the media type
			this.mediaType = MediaType.valueOf((String) properties.get(XivelyClient.XIVELY_MEDIA_TYPE));
			...
			Object eventFeed = properties.get(XivelyClient.XIVELY_DEFAULT_EVENT_FEED);
			this.defaultEventFeed = (eventFeed instanceof String) ? Integer.valueOf((String) eventFeed)
					: (Integer) eventFeed;
			
			// get the base feed, if needed
			Object alertFeed = (String) properties.get(XivelyClient.XIVELY_DEFAULT_ALERT_FEED);
			this.defaultAlertFeed = (alertFeed instanceof String) ? Integer.valueOf((String) alertFeed)
					: (Integer) alertFeed;
			...
			try
			{
				...
				this.waitingListSize = Integer.valueOf((String) properties.get(XivelyClient.XIVELY_WAITING_LIST_SIZE));
				this.waitingListSize = Math.min(this.waitingListSize, this.maxListSize);
				...
				this.selfTune = Boolean.valueOf((String) properties.get(XivelyClient.XIVELY_DELIVERY_QUEUE_SELFTUNE));
			}
			...
			this.alertQueue = new RESTDeliveryQueue(this.logger, this, this.mediaType);
			this.eventQueue = new RESTDeliveryQueue(this.logger, this, this.mediaType);
			...
			// check the key
			if (((this.key == null) || (this.key.isEmpty())) && ((this.serial != null) && (!this.serial.isEmpty()))
					&& ((this.productSecret != null) && (!this.productSecret.isEmpty())))
			{
				// activate the device
		...
		if (activationData != null)
		{
			...
			if ((apiKey != null) && (!apiKey.isEmpty()) && (feedId != 0))
			{
				...
				if (configurationAdminReference != null)
				{
					...
					try
					{
						Configuration configuration = confAdmin.getConfiguration(XivelyClient.class.getPackage()
								.getName());
						
						Dictionary<String, Object> properties = configuration.getProperties();
						
						properties.put(XivelyClient.XIVELY_DEFAULT_ALERT_FEED, feedId.toString());
						properties.put(XivelyClient.XIVELY_DEFAULT_EVENT_FEED, feedId.toString());
	 ...
	 * @param queue
	 */
	private void handleRestNotification(Integer eventFeed, GenericEvent innerEvent,
			Hashtable<String, Set<Datapoint>> waitingList, RESTDeliveryQueue queue)
	{
		...
		if ((eventFeed != null) && (innerEvent.getValue() != null))
		{
			...
			Set<Datapoint> datapointSet = waitingList.get(innerEvent.getStreamName());
			...
			// debug
			this.logger.log(LogService.LOG_DEBUG, XivelyClient.logId + "Event count for " + innerEvent.getStreamName()
					+ "=" + datapointSet.size() + "\n" + datapointSet);
			// check the list size
			if (datapointSet.size() >= this.waitingListSize)
			{
				...
				if (!this.existingChannels.contains(innerEvent.getStreamName()))
				{
					try
					{
						...
						XivelyService.instance().datastream(eventFeed).get(innerEvent.getStreamName());
				...
				DatapointInfo data = new DatapointInfo(new Integer(eventFeed), innerEvent.getStreamName(), datapointSet);
	...
	{
		return Math.max(this.waitingEventList.size(), this.waitingAlertList.size());
	}
