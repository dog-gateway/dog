...
public class ZWaveTemperatureAndHumiditySensorDriver implements Driver, ManagedService
{
	...
	private Vector<ZWaveTemperatureAndHumiditySensorDriverInstance> connectedDrivers;
	...
	private Set<String> temperatureAndHumidityDeviceCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveTemperatureAndHumiditySensorDriverInstance>();
		
		// initialize the set of implemented device categories
		temperatureAndHumidityDeviceCategories = new HashSet<String>();
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveTemperatureAndHumiditySensorDriver.class.getClassLoader()
						.loadClass(deviceCategory)))
				{
					...
					if (deviceCategory != null)
					{
						if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
								&& (temperatureAndHumidityDeviceCategories.contains(deviceCategory))
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveTemperatureAndHumiditySensorDriverInstance driverInstance = new ZWaveTemperatureAndHumiditySensorDriverInstance(
	...
	{
		for (Class<?> devCat : ZWaveTemperatureAndHumiditySensorDriverInstance.class.getInterfaces())
		{
			temperatureAndHumidityDeviceCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						...
						+ ZWaveTemperatureAndHumiditySensorDriver.class.getName());
			}
