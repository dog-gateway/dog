...
public class ZWaveSinglePhaseElectricityMeterDriver implements Driver, ManagedService
{
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveSinglePhaseElectricityMeterDriver.class.getClassLoader()
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveSinglePhaseElectricityMeterInstance driverInstance = new ZWaveSinglePhaseElectricityMeterInstance(
				network.get(), device, Integer.parseInt(sNodeID), instancesId, this.gateway.get()
						.getSpecificGateway(gateway).getNodeInfo().getDeviceNodeId(), updateTimeMillis, context);
