...
public class ZWaveSinglePhaseElectricityMeterDriver implements Driver, ManagedService
{
	...
	public static String LOG_ID = "[ZWaveSinglePhaseElectricityMeterDriver]: ";
	
	// String identifier for driver id
	public static final String DRIVER_ID = "ZWave_ZWaveSinglePhaseElectricityMeter_driver";
	...
	private Vector<ZWaveSinglePhaseElectricityMeterInstance> connectedDrivers;
	...
	private Set<String> singlePhaseElectricityMeterCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveSinglePhaseElectricityMeterInstance>();
		
		// initialize the set of implemented device categories
		singlePhaseElectricityMeterCategories = new HashSet<String>();
	...
	public int match(ServiceReference reference) throws Exception
	{
		...
		if (deviceCategory != null)
		{
			if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
					&& (singlePhaseElectricityMeterCategories.contains(deviceCategory))
					&& (this.gateway.get().isGatewayAvailable(gateway)))
			{
				matchValue = SinglePhaseElectricityMeter.MATCH_MANUFACTURER + SinglePhaseElectricityMeter.MATCH_TYPE;
			}
	...
	public String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveSinglePhaseElectricityMeterInstance driverInstance = new ZWaveSinglePhaseElectricityMeterInstance(
				network.get(), (ControllableDevice) context.getService(reference), Integer.parseInt(sNodeID),
				instancesId, this.gateway.get().getSpecificGateway(gateway).getNodeInfo().getDeviceNodeId(),
	...
	{
		for (Class<?> devCat : ZWaveSinglePhaseElectricityMeterInstance.class.getInterfaces())
		{
			singlePhaseElectricityMeterCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						...
						+ ZWaveSinglePhaseElectricityMeterDriver.class.getName());
			}
