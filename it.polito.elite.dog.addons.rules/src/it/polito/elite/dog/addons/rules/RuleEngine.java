...
public class RuleEngine implements ManagedService, RuleEngineApi, EventHandler
{
	...
	public void activate(BundleContext context)
	{
		...
		// debug
		this.logger
				.log(LogService.LOG_DEBUG, RuleEngine.logId + "Activated...");
	...
	public synchronized StatefulKnowledgeSession initRuleEvaluationSession()
	{
		...
		StatefulKnowledgeSession runtimeRuleSession = this.ruleBase
		...
		this.logger.log(LogService.LOG_DEBUG, RuleEngine.logId
				+ " runtime session created... " + runtimeRuleSession);
	...
	private void registerServices()
	{
		...
		p.put(EventConstants.EVENT_TOPIC, new String[] {
				...
				"it/polito/elite/domotics/model/notification/*" });
		this.srEventHandler = this.context.registerService(
				EventHandler.class.getName(), this, p);
		this.srDogRulesService = this.context.registerService(
				RuleEngineApi.class.getName(), this, null);
	...
	private synchronized void saveRules(File fileToSave)
	{
		...
		try
		{
			...
			JAXBContext ctx = JAXBContext.newInstance(RuleList.class
					.getPackage().getName());
	...
	public void handleEvent(Event event)
	{
		...
		StatefulKnowledgeSession runtimeRuleSession = this
				.initRuleEvaluationSession();
		if (runtimeRuleSession != null)
		{
			// Received a generic MonitorAdmin event (No MonitoringJob, so
			// mon.listener.id property is null)
			if (eventTopic != null
					&& eventTopic.equals("org/osgi/service/monitor")
					&& event.getProperty("mon.listener.id") == null)
			{
				...
				{
					this.logger.log(LogService.LOG_ERROR, RuleEngine.logId
							+ " device status deserialization error", e);
				}

				// If the deserialization works
				if (currentDeviceStatus != null)
				{
					Map<String, State> allStates = currentDeviceStatus
							.getStates();
					for (State cState : allStates.values())
					{
						// store the received notification
						receivedNotification = new StateChangeNotification(
								cState);
						receivedNotification.setDeviceUri(currentDeviceStatus
								.getDeviceURI());

						// debug
						this.logger.log(LogService.LOG_DEBUG, RuleEngine.logId
								+ " handling state change notification(s): "
			...
			// TimedTriggerNotification (DogScheduler)
					...
					&& (eventTopic
							.startsWith("it/polito/elite/domotics/model/notification/") || eventTopic
							.equals(TimedTriggerNotification.notificationTopic)))
			{
				...
				if (eventContent instanceof Notification)
				{
					...
					// debug
					this.logger
							.log(LogService.LOG_DEBUG, RuleEngine.logId
	...
	private Object getCurrentStateOf(String deviceURI, String stateClass)
	{
		...
		StatusVariable deviceStatusVariable = monitorAdmin
				.getStatusVariable(deviceURI + "/status");
			...
			Object[] params, Long startTimeS, Long endTimeS, Long afterTimeS)
	{
		...
		if (nowL < endTime)
		{
			logger.log(LogService.LOG_INFO, "Command Execution on " + toDevice
					+ " -> " + commandName);
