...
/**
 * This thread implements a queue. Every deltaT =
 * ZWaveDriverImpl.getPollingTimeMillis(), it wakes-up and asks Z-Way server for
 * system update. Later it checks TODO successivamente controlla quali device
 * devono essere triggerati in modo da far aggiornare il loro stato. Questo e'
 * dovuto al fato che alcuni device non comunicano correttamente il loro cambio
 * di stato al server zway, ed e' quindi necessario che venga effettuata su di
 * loro una GET da parte di ZWay. Questo thread si occupa di schedulare anche
 * questo tipo di azione. Per aggiunger un device alla lista dei nodi da
 * triggerare usare la addDeviceToQueue
 * 
 */
public class ZWavePoller extends Thread
{
	...
	public void run()
	{
		...
		{
			// log
			this.logger.log(LogService.LOG_DEBUG, ZWavePoller.logId
			...
			for (TriggerElem elem : deviceSet.values())
			{
				...
				if (elem.getTriggerTime() <= currentDate.getTime())
				{
					...
					elem.setTriggerTime(elem.getTriggerTime()
							+ elem.getUpdateTimeMillis());
				}
			}
			...
			// since this can be a time intensive inner task, yield to other
			// processes
			Thread.yield();
			...
			{
				// log the error
				this.logger.log(LogService.LOG_WARNING, ZWavePoller.logId
	...
	public void addDeviceToQueue(ZWaveNodeInfo nodeInfo, int updateTimeMillis)
	{
		...
		Date date = new Date();
		TriggerElem elem = new TriggerElem(nodeInfo, date.getTime()
