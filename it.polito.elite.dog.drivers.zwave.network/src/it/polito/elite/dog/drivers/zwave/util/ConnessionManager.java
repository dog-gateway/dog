...
public class ConnessionManager
{
	...
	private ConnessionManager(String sURL, String sUser, String sPassword, BundleContext bundleContext)
	{
		this.sURL = sURL;
		this.sUser = sUser;
		...
		HostnameVerifier hv = new javax.net.ssl.HostnameVerifier() {
			...
			{
				if (hostname.equals("z-cloud.z-wave.me"))
				{
					return true;
				}
				return false;
		 ...
		 */
		client = ClientBuilder.newBuilder().sslContext(ctx).register(JacksonFeature.class).build();
	 ...
	 * @return ConnessionManager
	 */
	public static ConnessionManager get(String sURL, String sUser, String sPassword, BundleContext bundleContext)
	{
		// create a new instance if needed, or returns the current one
		if (connessionManager == null || !sURL.equals(connessionManager.getURL())
				|| !sUser.equals(connessionManager.getUser()) || !sPassword.equals(connessionManager.getPassword()))
			connessionManager = new ConnessionManager(sURL, sUser, sPassword, bundleContext);
	...
	public ZWaveModelTree updateDevices(long lSince) throws Exception
	{
		Response response = service.path(DATA_PATH).path(String.valueOf(lSince))
				.request(MediaType.APPLICATION_JSON_TYPE).header("Authorization", "Basic " + getAuthToken()).post(null); // post
																															// is
																															// mandatory
	...
	public String sendCommand(String sCommand) throws ConfigurationException
	{
		...
		Response response = service.path(RUN_PATH).path(sCommand)
				//
				.request(MediaType.APPLICATION_JSON_TYPE).header("Authorization", "Basic " +this.authToken)
				.post(null); // post obbligatoria
		...
		{
			throw new ConfigurationException(null, "Can't read json from Z-Way server: " + response.toString());
		}
	...
	{
		byte[] btToken = Base64.encode((this.getUser() + ":" + getPassword()).getBytes());
