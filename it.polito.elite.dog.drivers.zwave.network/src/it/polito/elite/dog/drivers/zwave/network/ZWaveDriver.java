...
public abstract class ZWaveDriver implements DogDriver
{
	...
	// the endpoint address associated to this device by means of the gateway attribute
	...
	// UNIX timestamp of the last communication from the device. Its value comes from Z-Way server
	// and represents the real time of when a value is received. This is due because Z-Way server
	...
	// Init as 0. In this way we will probably receive a wrong value on first read, 
	...
	// counter used to manage warning in log due to no response from device after a Get 
	 ...
	 *            //TODO
	 */
	public ZWaveDriver(ZWaveNetwork network, ControllableDevice device, int deviceId, Set<Integer> instancesId, 
			int gatewayNodeId, int updateTimeMillis, BundleContext context)
	{
		...
		//Create the nodeInfo object. It defines nodeId, instancesId and Get command class 
	...
	private void fillConfiguration()
	{
		...
		for (DogElementDescription parameter : commandsSpecificParameters)
		{
			...
			String registerAddress = params.get(ZWaveInfo.REGISTER_ADDRESS);

			// get the Modbus register unit of measure
			String unitOfMeasure = params.get(ZWaveInfo.REGISTER_UOM);

			// get the Modbus register type
			String registerType = params.get(ZWaveInfo.REGISTER_TYPE);

			// get the Modbus register slave id
			String registerSlaveId = params.get(ZWaveInfo.SLAVE_ID);

			// get the Modbus register scale factor if needed
			String scaleFactor = params.get(ZWaveInfo.SCALE_FACTOR);
			*/
		...
		for (DogElementDescription parameter : notificationsSpecificParameters)
		{
			...
			*/
			...
			Set<CmdNotificationInfo> notificationNames = register2Notification.get(nodeInfo);
	...
	/*
	/**
	 * Update the data structure associated with this device from the network
	 *
	public void updateDeviceData()
	{
		network.read(getNodeInfo());
	}*/
	...
	/**
	 * Notifies a device-specific driver of new data coming from the
	 * underlying zwave network connection. The updated value is contained in the given
	 * {@link IDevice} instance.
	 * 
	 * @param deviceNode the {@link Device} instance representing the driver node.
	 * @param instanceNode the {@link Instance} instance representing the instance node.
	 * @param controllerNode the {@link Controller} instance representing the controller node.
	 * @param string
	 */
	public abstract void newMessageFromHouse(Device deviceNode, Instance instanceNode, 
	 ...
	 * sNodeId with the proper {@link ZWaveDriverImpl}
	 * instance.
	 * 
	 * @param sNodeId the node to add.
	 */
	...
	/**
	 * Extending classes might implement this method
	 * to define the {@link ZWaveNodeInfo} associated with the driver
	 * @param deviceId
	 * @param instancesId 
	 * @param isController
	 * @return {@link ZWaveNodeInfo}
	 */
	protected abstract ZWaveNodeInfo createNodeInfo(int deviceId, Set<Integer> instancesId, boolean isController);
}
