...
/**
 * This class provides multiple utilities for the Device Manager bundle.
 * 
...
public class Util
{
	...
	public static boolean isDevice(ServiceReference<?> ref)
	{
		try
		{
			Filter device = createFilter("(|(%s=%s)(%s=%s))",
					new Object[] { Constants.OBJECTCLASS, Device.class.getName(),
	...
	public static boolean isDeviceInstance(ServiceReference<?> ref)
	{
		try
		{
			Filter device = createFilter("(%s=%s)", new Object[] { Constants.OBJECTCLASS, Device.class.getName() });
			return device.match(ref);
		}
		catch (Exception e)
		{
