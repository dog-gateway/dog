...
public class ZWaveMovementSensorDriver implements Driver, ManagedService
{
	...
	public static String LOG_ID = "[ZWaveMovementSensorDriver]: ";
	
	// String identifier for driver id
	public static final String DRIVER_ID = "ZWave_ZWaveMovementSensor_driver";
	...
	private Vector<ZWaveMovementSensorDriverInstance> connectedDrivers;
	...
	private Set<String> movementSensorCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveMovementSensorDriverInstance>();
		
		// initialize the set of implemented device categories
		movementSensorCategories = new HashSet<String>();
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveMovementSensorDriver.class.getClassLoader().loadClass(
						deviceCategory)))
				{
					...
					if (deviceCategory != null)
					{
						if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
								&& (movementSensorCategories.contains(deviceCategory))
								&& (this.gateway.get().isGatewayAvailable(gateway)))
						{
							matchValue = MovementSensor.MATCH_MANUFACTURER + MovementSensor.MATCH_TYPE;
						}
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveMovementSensorDriverInstance driverInstance = new ZWaveMovementSensorDriverInstance(network.get(), device,
	...
	{
		for (Class<?> devCat : ZWaveMovementSensorDriverInstance.class.getInterfaces())
		{
			movementSensorCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						+ " not defined in configuraton file for " + ZWaveMovementSensorDriver.class.getName());
			}
