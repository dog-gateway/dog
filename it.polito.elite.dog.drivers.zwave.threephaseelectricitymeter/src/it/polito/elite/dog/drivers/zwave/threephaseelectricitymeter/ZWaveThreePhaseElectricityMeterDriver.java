...
public class ZWaveThreePhaseElectricityMeterDriver implements Driver, ManagedService
{
	...
	public String attach(ServiceReference reference) throws Exception
	{
		...
		Set<String> nodeIdSet = ((ControllableDevice) this.context.getService(reference)).getDeviceDescriptor()
				.getSimpleConfigurationParams().get(ZWaveInfo.NODE_ID);
		
		// get the corresponding end point set
		Set<String> instanceIdSet = ((ControllableDevice) this.context.getService(reference)).getDeviceDescriptor()
		...
		ZWaveThreePhaseElectricityMeterInstance driverInstance = new ZWaveThreePhaseElectricityMeterInstance(
				this.network.get(), (ControllableDevice) this.context.getService(reference), Integer.parseInt(sNodeID),
				instancesId, this.gateway.get().getSpecificGateway(gateway).getNodeInfo().getDeviceNodeId(),
				this.updateTimeMillis, this.context);
		
		((ControllableDevice) this.context.getService(reference)).setDriver(driverInstance);
	...
	{
		if ((this.gateway.get() != null) && (this.network.get() != null) && (this.context != null) && (this.regDriver == null))
		{
