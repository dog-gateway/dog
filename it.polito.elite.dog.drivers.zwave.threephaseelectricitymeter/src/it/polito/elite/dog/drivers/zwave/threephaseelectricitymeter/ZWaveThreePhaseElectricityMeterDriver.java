...
public class ZWaveThreePhaseElectricityMeterDriver implements Driver,
		ManagedService
{
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if (deviceCategory != null)
		{
			if (manifacturer != null
					...
					&& (threePhaseElectricityMeterCategories
			...
			throws Exception
	{
		...
		ZWaveThreePhaseElectricityMeterInstance driverInstance = new ZWaveThreePhaseElectricityMeterInstance(
				...
				Integer.parseInt(sNodeID), instancesId, this.gateway.get()
						...
						.getDeviceNodeId(), this.updateTimeMillis, this.context);
	...
	{
		for (Class<?> devCat : ZWaveThreePhaseElectricityMeterInstance.class
				.getInterfaces())
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME,
						ZWaveInfo.PROPERTY_UPDATETIME
								...
								+ ZWaveThreePhaseElectricityMeterDriver.class
										.getName());
			}
