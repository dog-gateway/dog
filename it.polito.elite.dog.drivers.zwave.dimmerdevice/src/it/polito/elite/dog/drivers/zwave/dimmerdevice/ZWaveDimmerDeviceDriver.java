...
public class ZWaveDimmerDeviceDriver implements Driver, ManagedService
{
	...
	public static final String DRIVER_ID = "ZWave_ZWaveDimmableDevice_driver";
	...
	private Vector<ZWaveDimmerDeviceDriverInstance> connectedDrivers;
	...
	private Set<String> dimmableDeviceCategories;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveDimmerDeviceDriverInstance>();
		
		// initialize the set of implemented device categories
		dimmableDeviceCategories = new HashSet<String>();
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveDimmerDeviceDriver.class.getClassLoader().loadClass(
						deviceCategory)))
				{
					...
					if (deviceCategory != null)
					{
						if (manifacturer != null && (gateway != null) && (manifacturer.equals(ZWaveInfo.MANUFACTURER))
								&& (dimmableDeviceCategories.contains(deviceCategory))
								&& (this.gateway.get().isGatewayAvailable(gateway)))
						{
							matchValue = DimmableLight.MATCH_MANUFACTURER + DimmableLight.MATCH_TYPE;
						}
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveDimmerDeviceDriverInstance driverInstance = new ZWaveDimmerDeviceDriverInstance(this.network.get(),
				device, Integer.parseInt(sNodeID), instancesId, this.gateway.get().getSpecificGateway(gateway)
						.getNodeInfo().getDeviceNodeId(), this.updateTimeMillis, this.stepPercentage, context);
	...
	{
		for (Class<?> devCat : ZWaveDimmerDeviceDriverInstance.class.getInterfaces())
		{
			dimmableDeviceCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						+ " not defined in configuraton file for " + ZWaveDimmerDeviceDriver.class.getName());
			}
