...
public class ZWaveOnOffDeviceDriver implements Driver, ManagedService
{
	...
	public static String LOG_ID = "[ZWaveOffDeviceDriver]: ";
	
	// String identifier for driver id
	public static final String DRIVER_ID = "ZWave_ZWaveOnOffDevice_driver";
	...
	private Vector<ZWaveOnOffDeviceDriverInstance> connectedDrivers;
	...
	public void activate(BundleContext bundleContext)
	{
		...
		connectedDrivers = new Vector<ZWaveOnOffDeviceDriverInstance>();
		
		// initialize the set of implemented device categories
		onOffDeviceCategories = new HashSet<String>();
	...
	public synchronized int match(ServiceReference reference) throws Exception
	{
		...
		if ((this.network != null) && (this.gateway != null) && (this.regDriver != null))
		{
			...
			try
			{
				...
				if (Controllable.class.isAssignableFrom(ZWaveOnOffDeviceDriver.class.getClassLoader().loadClass(
						deviceCategory)))
				{
	...
	public synchronized String attach(ServiceReference reference) throws Exception
	{
		...
		ZWaveOnOffDeviceDriverInstance driverInstance = new ZWaveOnOffDeviceDriverInstance(network.get(), device,
	...
	{
		for (Class<?> devCat : ZWaveOnOffDeviceDriverInstance.class.getInterfaces())
		{
			onOffDeviceCategories.add(devCat.getName());
		}
	}
		...
		if (properties != null)
		{
			...
			else
			{
				throw new ConfigurationException(ZWaveInfo.PROPERTY_UPDATETIME, ZWaveInfo.PROPERTY_UPDATETIME
						+ " not defined in configuraton file for " + ZWaveOnOffDeviceDriver.class.getName());
			}
