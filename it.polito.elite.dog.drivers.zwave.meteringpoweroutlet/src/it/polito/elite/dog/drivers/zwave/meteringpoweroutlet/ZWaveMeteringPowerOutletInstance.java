...
import it.polito.elite.domotics.model.devicecategory.EnergyMeteringPowerOutlet;
import it.polito.elite.domotics.model.devicecategory.MeteringPowerOutlet;
import it.polito.elite.domotics.model.devicecategory.PowerMeteringPowerOutlet;
...
public class ZWaveMeteringPowerOutletInstance extends ZWaveDriver implements MeteringPowerOutlet
{
	// the class logger
	...
	{
		// debug
		logger.log(LogService.LOG_DEBUG, ZWaveMeteringPowerOutletDriver.LOG_ID + "Device " + device.getDeviceId()
	...
	public void newMessageFromHouse(Device deviceNode, Instance instanceNode, Controller controllerNode, String sValue)
	{
		...
		else if (lastUpdateTime < updateTime)
		{
			...
			notifyNewActiveEnergyValue(DecimalMeasure.valueOf(activeEnergy+" "+SI.KILO(SI.WATT.times(NonSI.HOUR)).toString()));
			
			double activePower = Double.valueOf(ccElectricityEntry.get("2").getDataElemValue("val").toString());
			notifyNewActivePowerValue(DecimalMeasure.valueOf(activePower+" "+SI.WATT.toString()));
		}
		/*else
		{
			// increment counter
			...
			if (nFailedUpdate >= 5)
			{
				logger.log(LogService.LOG_WARNING,
						ZWaveMeteringPowerOutletDriver.LOG_ID + "Device " + device.getDeviceId()
		...
		int nLevel = 0;
		CommandClasses ccEntryOnOff = instanceNode.getCommandClasses().get(ZWaveAPI.COMMAND_CLASS_SWITCH_BINARY);
	...
	public void notifyNewActivePowerValue(Measure<?, ?> powerValue)
	{
		...
		currentState.setState(SinglePhaseActivePowerMeasurementState.class.getSimpleName(), new SinglePhaseActivePowerMeasurementState(
				pValue));
